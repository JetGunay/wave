  ...
  export default {
    ...
    methods: {
      ...
      filter(id, arr) {
        ...
        if(arr.title === 'Today' && id !== '') {
          const today = new Date().toISOString().slice(0, 10);
          const query = `tasks?responsibles=[${userids}]&fields=[responsibleIds, parentIds, superParentIds]&scheduledDate=${today}&customStatuses=["IEADVMHZJMBRONOY"]`;
          this.fetch(id, query);
          console.log(arr)
        } else {
